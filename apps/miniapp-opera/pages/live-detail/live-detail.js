// 直播详情：三态（live 直播中 / scheduled 倒计时 / ended 回顾）
const liveItems = [
...
Page({
  ...
  onLoad(options) {
    ...
    this.setData({ item, statusLabel: STATUS_LABELS[item.status] || '状态未知' });
    if (item.status === 'scheduled') this.startCountdown(item.startAt);
  },
  ...
  onNotify() {
    ...
    wx.showToast({ title: '已设置开播提醒（演示）', icon: 'success' });
  },
  ...
  retry() {
    if (this.id) this.onLoad({ id: this.id });
  },
